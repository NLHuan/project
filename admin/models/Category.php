...
VALUES (:name, :avatar, :description, :status)";
    ...
    INNER JOIN products ON categories.id = products.category_id where categories.id = $id");
        $arr_select = [];
        $obj_delete_product->execute($arr_select);
        $products = $obj_delete_product->fetchAll(PDO::FETCH_ASSOC);

        return $products;
    }
    public function getTwoID($id)
    {
        $obj_delete_product = $this->connection
            ->prepare("SELECT * FROM categories 
    INNER JOIN products ON categories.id = products.category_id where categories.id = $id");
    ...
    public function getAllPagination($params = [])
    {
        ...
        $start = ($page - 1) * $limit;
        $obj_select = $this->connection
            ->prepare("SELECT * FROM categories WHERE TRUE $this->str_search
    ...
    {
        $obj_select = $this->connection
            ->prepare("SELECT COUNT(id) FROM categories WHERE TRUE $this->str_search");
        $obj_select->execute();
        return $obj_select->fetchColumn();
    }
}
